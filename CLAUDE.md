...
| Every CNS transcript fails with `400 invalid_request_error` naming `output_config.format.schema` | The structured-outputs API REJECTS some JSON-schema keywords. `maxItems` on an array is one: "For 'array' type, property 'maxItems' is not supported". Hit live 2026-09-10 -- it failed the first five transcripts of the Q2 backfill before a single finding was produced. NO offline test could catch it: every test injects a fake caller via `call_fn`, so the schema is never validated by the API | Fixed @2.35.2 -- `maxItems` removed; the finding cap is requested in the prompt's prose and ENFORCED in `screen_transcript`, which truncates after parsing. `test_findings_schema_uses_no_api_rejected_keywords` walks the schema tree and fails if any rejected keyword returns. NEVER add a schema keyword without probing it live first |
