...
| A long YouTube video fails with `duration NNNNs exceeds cap (3600s)` | Only the yt-dlp/STT path is duration-capped (`LEARN_STT_MAX_DURATION_SEC`); reaching it at all means the captions path found nothing | Captions have NO duration cap -- a 2h video transcribes fine when it has any caption track. If you see this, the real question is why captions missed: check the log line above it for the track list |
