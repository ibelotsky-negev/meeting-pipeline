...
| `/pulse/trigger` | Manual weekly pulse (`?dry_run=&days=N&force=`). REPLAY: `?end=YYYY-MM-DD&to=a@b.com` re-analyzes a PAST window and mails it to a specific address -- a replay skips the archive and the briefing-book updates, and bypasses the weekly lock (see Weekly Pulse) |
...
| `/cns/run` | Manual CNS earnings screen run (`?dry_run=&days=&limit=&backlog=&force=&reconcile=&sync=`) |
...
- **Weekly Pulse:** Sunday 22:00 IST, scans all team emails/Teams/meetings, 4-pass Claude analysis, Green/Yellow/Red report emailed to Ken. Scope is set by two domain lists in [config.py](config.py) that are DELIBERATELY DIFFERENT SIZES, and conflating them is the trap: `PULSE_DOMAINS` is the narrow SCAN set (which mailboxes and Teams chats to enumerate) and holds only `palomar-labs.com` + `ariadnebio.com`; `PULSE_TEAM_DOMAINS` is the wider MEMBERSHIP set (which addresses count as team on an email's From/To and on a Fireflies meeting) and still holds `negevlabs.com` + `zirmania.com`, because `bk@negevlabs.com` remains the canonical downstream address even though that domain is no longer scanned. Narrow the scan set freely; removing a domain from the membership set silently reclassifies real team mail as external. Each pass parses Claude JSON through `_pulse_parse_json`, which tolerates prose and fenced payloads -- see Common Failure Modes before loosening it. Extraction passes 1-3 run through `_pulse_run_chunked_pass`: the corpus is packed into as many calls as `PULSE_MAX_INPUT_CHARS` allows (item-aligned, never mid-email) and the per-chunk signals merged, so the WHOLE week is analyzed -- the real 602-email run needs 4 calls. Synthesis (pass 4-5) is deliberately NOT chunked: it reasons over the merged signal JSON and must see all of it at once. A REPLAY run (`end=` and/or `to=` on `/pulse/trigger`) re-analyzes a past window for comparison: it sends the report to the override recipients but does NOT archive (that would overwrite the original week's stored report -- the thing being compared against) and does NOT apply briefing-book updates (the original run already applied them; re-applying double-counts)
...
them, inherited CC preserved -- but a named recipient is RESOLVED to a real
address first by `_resolve_recipients`, because the intake parser is asked
for the people the sender named and a sender names PEOPLE, not addresses
(a live watch held `["Elana"]`): an entry that is already an address passes
through, a bare name is matched against the thread's own reply-all
recipients by display name or address local part, and anything
unresolvable is DROPPED -- an empty result leaves the inherited reply-all
recipients untouched rather than addressing a draft to an unroutable word) and sends the owner ONE report email per run:
...
- **Daily flow (`run_daily`):** discover new transcripts from the global feed
  ...
  `run_season` (15th of Mar/Jun/Sep/Dec) reads ONLY the stored findings for
...
- **State** on `/data`: `cns_ledger.json` (per-period status, terminal states
  never reprocessed, retryable gaps capped at `CNS_MAX_FETCH_ATTEMPTS`),
...
- **Endpoints:** `/cns/run`, `/cns/season` (both `?sync=` for inline JSON,
  ...
  Cron: daily 07:00 and season 15th of Mar/Jun/Sep/Dec 08:00, both
  Asia/Jerusalem. CLI is not provided; use the routes.
...
| /pulse/check shows false | Missing Azure AD permissions | Ken must grant + admin consent. Read `teams_ready` vs `mail_ready` to see WHICH path is down, and `diagnostics` for the HTTP code |
| Pulse report is missing a whole input class (all meetings, or all Teams messages) even though the collector logged a non-zero count | A Claude pass returned its JSON wrapped in prose, and `_pulse_parse_json` only stripped a markdown fence when the reply STARTED with one -- so `json.loads` choked on the prose and the pass returned empty lists. Logged only as `Failed to parse Claude JSON` + `Pass N complete: 0G 0Y 0R`; the extracted signal was correct and thrown away. Hit Pass 3 on 2026-08-30 (archive `2026-W35`), losing both meetings from the report | Fixed @2.30.1 -- `_pulse_json_candidates` tries the whole text, then any fenced block anywhere, then the outermost brace span, and the failure log now carries the raw head so it is diagnosable. Applies to ALL passes, not just meetings |
| `Teams messages scanned: 0` in the pulse | Two independent causes, and before 2.30.1 neither left a log line. (a) Channel path: `/groups` 403s without `Group.Read.All`, so zero Teams are discovered. (b) Chat path: `/users/{id}/chats` succeeds but the per-chat `/chats/{id}/messages` read was a bare `continue` on non-200 | Fixed @2.30.1 -- both paths log the status code (chats summarized once per user, not once per chat). Check the logs, then `/pulse/check`. Note 0 can also be CORRECT: the team runs on email + Zoom/Google Meet, and Teams Graph transcript access is disabled tenant-wide |
| `/pulse/check` reports `ChannelMessage.Read.All: false` with `channels HTTP 400: Query option 'Top' is not allowed` | Graph rejects `$top` on `/teams/{id}/channels` (it is allowed on `/groups`, `/chats` and `/messages`, which is what makes this easy to get wrong). Shipped in the 2.30.1 probe; the production collector never had it | Fixed @2.31.1. When adding a Graph probe, mirror the production call EXACTLY rather than writing a tidier version of it |
| Pulse misses things that were plainly in the week's email, with no error anywhere | Pre-2.31.0 each pass was ONE call hard-truncated at `PULSE_MAX_INPUT_CHARS`. The 2026-08-30 run logged `Truncating input from 233726 to 80000 chars` -- 66% of the corpus never reached the model, and the report looked complete. The briefing book (~10.7K chars) also rides in the system prompt and was never counted against the budget | Fixed @2.31.0 -- `_pulse_run_chunked_pass` splits the corpus across calls and merges the signals; `_pulse_input_budget` now subtracts both the prompt scaffold and the briefing book. A `Truncating input` WARNING should now only ever appear for a SYNTHESIS pass -- if it does, signal is being lost: raise `PULSE_MAX_INPUT_CHARS` |
| A team member's mail or meetings are missing from the pulse | Their domain is not in the right list. `PULSE_DOMAINS` = the SCAN set (mailboxes + Teams chats enumerated); `PULSE_TEAM_DOMAINS` = the MEMBERSHIP set (addresses that count as team on mail From/To and on meetings). Both in [config.py](config.py) | Add a genuinely new working domain to BOTH. Adding to the scan set only is what makes a mailbox invisible; removing from the membership set is what makes real mail look external |
| `Teams messages scanned` looks plausible but low, and busy people are under-represented | Graph caps `$top` at 50 on `/chats` and `/messages`, and the pulse never followed `@odata.nextLink`. On 2026-09-01 seven accounts sat EXACTLY at 50 chats -- that is the cap, not the count | Fixed @2.31.2 (paging) and @2.32.0 (ordering). `_pulse_graph_collect` follows nextLink bounded by `PULSE_CHAT_PAGE_LIMIT` / `PULSE_MESSAGE_PAGE_LIMIT`, and the chat list is now requested `$orderby=lastMessagePreview/createdDateTime desc` with a `stop_when` predicate that ends the walk at the first chat outside the window -- so hitting a page limit is a non-event, not a silent loss, and an inactive chat costs no message call at all. `$orderby=lastUpdatedDateTime` is REJECTED (400); the default order happened to be descending already, but relying on that was luck. General rule: any `$top` on a Graph collection needs nextLink handling -- treat a count equal to `$top` as truncation |
| A pulse REPLAY reports more Teams traffic than the window contains | Teams collection had only a LOWER date bound -- `msg_date < start_iso` and nothing else. Invisible on the weekly cron, where the window ends at `now`, so it survived indefinitely; a 2026-09-01 replay of Aug 23-30 swept in two extra days. Emails (`receivedDateTime ge/le`) and Fireflies (`fromDate/toDate`) were bounded on both sides all along | Fixed @2.33.1 -- `_pulse_msg_in_window` bounds both ends and `pulse_collect_teams` threads `end_iso` down to both fetchers. `end_iso=None` keeps the old open-ended behavior. When adding a collector, bound the window on BOTH sides even if today's only caller ends at now |
| A pulse pass ignores its output format, or Ken's standing corrections stop taking effect | `_pulse_truncate_input` cuts the ASSEMBLED prompt from the END -- which is exactly where the output contract, the JSON schema, and the appended corrections block live. Live on 2026-09-02: Pass 5 was cut 93481->80000, so the model never saw `OUTPUT (JSON)` and wrote a markdown report BOTH times (the 2.33.2 retry appended the contract to the end, where it was cut again -- 93797->80000); Pass 4 lost 713 chars, the tail of its corrections block. Signals grew ~10x once truncation and domain scope were fixed, which is what pushed these passes over | Fixed @2.34.0 -- `_pulse_build_within_budget` trims the DATA (whole signal items, longest list first, never mid-JSON) until the ASSEMBLED prompt fits, so every instruction survives; corrections are built in BEFORE the budget is applied. RULE: never let a prompt's tail be the thing that gets cut -- put data in the middle and trim the data |
| A pulse pass reports `0G 0Y 0R` (or zero briefing proposals) with no parse error | The model answered a JSON request with prose. On 2026-09-01 Pass 5 returned a 17K-char markdown report and proposed zero updates. The 2.30.1 parser handles this cleanly -- but a caught failure is still a whole pass contributing nothing, and a silent zero looks exactly like a quiet week | Fixed @2.33.1 -- `_pulse_call_claude_json` retries ONCE with `PULSE_JSON_CONTRACT` appended, and logs an ERROR naming the pass if the retry also fails. The briefing prompt in [prompts.py](prompts.py) now ends with a hard output contract. Route every JSON-expecting pass through `_pulse_call_claude_json`, never bare `_pulse_parse_json(_pulse_call_claude(...))` |
| A Teams early-exit / `$orderby` change drops in-window messages, or 400s | Graph is ASYMMETRIC here and status 200 is not proof of ordering -- probed live 2026-09-01. `/users/{id}/chats`: `$orderby=lastMessagePreview/createdDateTime desc` works, `$orderby=lastUpdatedDateTime` is a 400. `/chats/{id}/messages`: `$orderby=createdDateTime desc` works AND is strictly descending (8/8) -- but the DEFAULT order is NOT, it sorts by `lastModifiedDateTime`, so an edited old message floats to the top and 3 of 6 sampled chats came back unsorted by `createdDateTime`. `/teams/{t}/channels/{c}/messages`: `$orderby` is a **400**, not supported at all | An early exit is only sound on an EXPLICITLY ordered walk. Chats and chat-messages have one (`stop_when` in `_pulse_graph_collect`); CHANNEL messages deliberately do NOT -- there is a test asserting the channel URL carries no `$orderby`. Never infer ordering from a 200, and never from a sample that happens to look sorted |
| Teams chats return `403 Forbidden / InsufficientPrivileges / AclCheckFailed` for SOME chats while others read fine | NOT a missing consent -- `Chat.Read.All` and `ChatMessage.Read.All` are both granted. Probed live 2026-09-01 across two accounts, and there are TWO unrelated causes, which is why one sample misleads: (a) `chatType` `unknownFutureValue` -- the enum placeholder for kinds v1.0 does not model -- always has ZERO members and always 403s; these are pseudo-chats with no content (all 6 of gene@'s refusals). (b) A federated chat HOSTED IN THE OTHER PARTY'S TENANT (all 3 of ken@'s refusals had external participants; external chats hosted in our tenant read fine) | (a) fixed @2.32.0 -- `PULSE_SKIP_CHAT_TYPES` skips them before the message call, so they cost nothing and no longer inflate the rate. (b) is NOT fixable by consent: you cannot grant this app access to another tenant's chat data. It is counted and reported in the `Teams coverage gap` line and `/pulse/debug` `teams.coverage`. A residual refusal rate is expected, not a bug -- investigate only if it jumps |
...
| Some meetings silently produce no Phase-1 email while others do; hits the whole team, looks random (2026-09-01: Fireflies recorded 5 meetings, Sara emailed 2) | **The poll is blind, so the webhook is the only path.** `FIREFLIES_API_KEY` is Ken's PERSONAL key: `user{}` -> bk@negevlabs.com, `users{}` -> that one member, `num_transcripts` 3, and a `transcripts` list query for the current day returns **0** -- nothing newer than 2026-08-26 is visible to it. So `get_recent_transcripts` (the 2.29.0 safety net AND the drain's trigger) can never discover a teammate's meeting, and `get_transcript_by_id` on a parked one 404s `object_not_found` for the same reason -- proven on Ken's own 2026-09-01 sync, still 404 after 3h and 3 drain attempts. Every notification therefore rides on the webhook landing AND the transcript being fetchable within ~45s; a webhook that never arrives (Kostia, same day) is lost with no backstop | NOT a code fix -- the API key needs workspace-wide transcript visibility. Check what changed in the Fireflies workspace/seat around 2026-08-26/27 (the date the key's view stops and the date team follow-ups stopped). Verify any fix with `transcripts(fromDate: <today>)` returning today's meetings, not just `user{}` |
...
Weekly Pulse Teams: `PULSE_SKIP_CHAT_TYPES` (chat kinds never read; `meeting` for join/leave noise, `unknownFutureValue` for memberless pseudo-chats that always 403). Paging: `PULSE_CHAT_PAGE_LIMIT` (pages of chats per user, default 6) and `PULSE_MESSAGE_PAGE_LIMIT` (pages of messages per chat or channel, default 4). Graph caps `$top` at 50 on `/chats`, `/messages` and `/channels`, so these bound how far `_pulse_graph_collect` follows `@odata.nextLink`. Hitting a limit is LOGGED, never silent.

Weekly Pulse: `PULSE_MAX_INPUT_CHARS` (per-call input ceiling, default 80000 ~= 20K tokens), `PULSE_MAX_CHUNKS` (calls per pass, default 12 -- a cost bound; exceeding it logs exactly how many items go unanalyzed, never silent), `PULSE_RATE_LIMIT_SECONDS` (pause between Claude calls, default 65 -- sized for a 30K input TPM ceiling). All three are read in [config.py](config.py); set `PULSE_RATE_LIMIT_SECONDS=0` and raise `PULSE_MAX_INPUT_CHARS` if the account moves to a higher tier, which is the single biggest lever on pulse wall-clock (~3 min of the run is pauses).
...
cns-earnings-screen: `FMP_API_KEY` (disables the whole module when unset), optional `CNS_LOOKBACK_DAYS` (daily discovery window, default 3), `CNS_MAX_TRANSCRIPTS_PER_RUN` (per-run cap, default 60), `CNS_TRANSCRIPT_GRACE_DAYS` (days after a reported call before missing-transcript is flagged, default 3), `CNS_MAX_FETCH_ATTEMPTS` (retry cap on a transcript gap, default 4), `CNS_UNIVERSE_TTL_DAYS` (universe cache refresh, default 7), `CNS_SCREEN_MODEL` / `CNS_SEASON_MODEL` (default claude-opus-5), `CNS_GATE_WINDOW` (BD-term proximity gate, default 600 chars), `CNS_ZONE_BAND` (Q&A position band, default "0.15,0.85"), `CNS_RECIPIENTS` (digest email, default bk@negevlabs.com,dan@negevlabs.com). Reuses `CLAUDE_API_KEY`, `BOT_SENDER_EMAIL`, `MS_GRAPH_*`.

## Current Known Issues

- **Fireflies `duration` is in MINUTES** -- confirmed against the live API on 2026-08-20 (real samples: 48, 11.32, 53.25, 38.69, 72.08). This resolved a long-standing contradiction: [fireflies_client.py](fireflies_client.py) added the field as minutes while [app.py](app.py) Weekly Pulse divided it by 60 as SECONDS, so a 48-minute meeting was described to Claude as "1min" in the pulse prompt. The Pulse side was wrong; the error was invisible because that value only ever reached an LLM prompt, and `create_hubspot_meeting` (the one place a wrong duration would show) is dead code, never called. Both call sites now share `duration_to_minutes`, which clamps to `MAX_MEETING_MINUTES` but deliberately does NOT try to detect seconds and rescale -- that would shrink a genuinely long recording 60x and drop it from the poll window.
...
- Weekly Pulse requires Azure AD **application** permissions: `Mail.Read`, `Chat.Read.All`, `ChannelMessage.Read.All`, and `Group.Read.All` (or `Directory.Read.All`) -- all granted + admin consented. `Group.Read.All` is the one that gates Teams CHANNEL discovery: `pulse_collect_teams` finds Teams via `/groups?$filter=...'Team'`, so without it that call 403s and the entire channel path silently collects nothing (confirmed live 2026-08-30). Verify with `/pulse/check`, which since 2.30.1 probes the real calls and reports `mail_ready` / `teams_ready` separately -- a Teams gap no longer hides a healthy mail path, and vice versa.
