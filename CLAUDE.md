...
| `Teams messages scanned` looks plausible but low, and busy people are under-represented | Graph caps `$top` at 50 on `/chats` and `/messages`, and the pulse never followed `@odata.nextLink`. On 2026-09-01 seven accounts sat EXACTLY at 50 chats -- that is the cap, not the count | Fixed @2.31.2 -- `_pulse_graph_collect` pages through, bounded by `PULSE_CHAT_PAGE_LIMIT` / `PULSE_MESSAGE_PAGE_LIMIT`, and logs when a bound is hit. Any `$top` on a Graph collection needs nextLink handling; treat a count equal to `$top` as truncation until proven otherwise |
| Teams chats return `403 Forbidden / InsufficientPrivileges / AclCheckFailed` for SOME chats while others read fine | Not a missing consent -- `Chat.Read.All` and `ChatMessage.Read.All` are both granted. `AclCheckFailed` means the app is not authorized on those particular chat resources. On 2026-09-01 roughly 45% of chats refused this way (e.g. gene@: 16 read / 16 refused). Suspected external or federated participants, NOT confirmed | Partly open. @2.31.2 `pulse_collect_teams` logs a `Teams coverage gap` summary with the refusal percentage and surfaces it in `/pulse/debug` under `teams.coverage`, so the gap is measurable instead of invisible. Closing it likely needs the Teams protected-API / RSC route |
...
Weekly Pulse Teams paging: `PULSE_CHAT_PAGE_LIMIT` (pages of chats per user, default 6) and `PULSE_MESSAGE_PAGE_LIMIT` (pages of messages per chat or channel, default 4). Graph caps `$top` at 50 on `/chats`, `/messages` and `/channels`, so these bound how far `_pulse_graph_collect` follows `@odata.nextLink`. Hitting a limit is LOGGED, never silent.
