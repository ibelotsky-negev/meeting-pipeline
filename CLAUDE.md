...
| A team member's mail or meetings are missing from the pulse | The pulse domain lists lagged the Palomar rename. `PULSE_DOMAINS` ([config.py](config.py)) picks which mailboxes are scanned AND which From/To addresses count as team; `PULSE_TEAM_DOMAINS` ([app.py](app.py)) filters Fireflies meetings | Fixed @2.30.1 -- both include `palomar-labs.com`. Add any new sending domain to BOTH |
